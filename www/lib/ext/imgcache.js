...
(function() {
	...
	var is_cordova = function() {
        return (cordova && !cordova.browser);
	};
	...
	ImgCache.cacheFile = function(img_src, success_callback, fail_callback) {
		...
		fileTransfer.download(
			...
			function(entry) {
                ...
                if (success_callback) success_callback(entry.toURL());
			},
			function(error) {
	...
	ImgCache.isCached = function(img_src, response_callback) {
		...
		if (!ImgCache.filesystem || !ImgCache.dirEntry || !response_callback) {
            logging('isCached called with not initialized data', 3);
			return;
        }
    ...
    ImgCache.deleteCachedFile = function(img_src, success_callback, error_callback) {
        if (!ImgCache.filesystem || !ImgCache.dirEntry) {
            logging('ImgCache not loaded yet!', 2);
            return;
        }
        ...
        ImgCache.filesystem.root.getFile(
            ...
            function(fileEntry) {
                fileEntry.remove(
                    function() {
                        logging('Cached file removed for: ' + img_src, 2);
                        success_callback();
                    },
                    function(err) {
                        logging('Cached file removing failed for: ' + img_src + " with error: " + err, 2);
                        error_callback();
                    });
            },
            function() {
                logging('Cached file not found for: ' + img_src + ", path = " + path, 2);
                error_callback();
            }
        );
    };
