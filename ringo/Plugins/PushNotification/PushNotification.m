...
- (void)didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken {
    ...
    // if we have pending notifications - send last of them to JS
    id lastMessage = [self.pendingNotifications lastObject];
    if (lastMessage) {
        NSLog(@"send stored notification: %@", lastMessage);

        [self didReceiveRemoteNotification:lastMessage];
    }
}
